...
def openTheOuputFile():
    ...
    file_obj = open(write_to_file_path, "a")  # "a" - Append - will append to the end of the file.
    return file_obj
...
def beforeDBProcessing(line):
    ...
    output_file.write(str(line + " " + current_time + "\n"))
    # output_file.flush()
    sensor_id = line[0:3]
    ...
    while True:
        ...
        if line == "KILL":
            ...
            if not isOK(line):
                ...
                if not isConnected():
                    file_obj = open("no_internet_output.txt", "a")
                    file_obj.write(str(line + current_time + "\n"))
                    file_obj.close()
                else:
                    import os
